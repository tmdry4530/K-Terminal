...
test('assessNewsTrade only allows real-time serious news for preview/review', () => {
  ...
  assert.equal(assessNewsTrade({ symbol: 'MKRUSDT', direction: 'SHORT', event_type: 'protocol_critical_exploit', verification_state: 'VERIFIED', confidence_score: 0.77, urgency_score: 0.8, trade_allowed: false, evidence_summary: [{ source_label: 'DefiLlama Hacks', summary: 'DefiLlama hack record: project=DAO Maker Vesting; classification=Protocol Logic; technique=Access Control Exploit', publisher_ts: '2021-09-04T00:00:00Z' }] }).status, 'NO_TRADE');
  const staleDefillama = parseInboxLine(JSON.stringify({ received_at: new Date().toISOString(), payload: { symbol: 'MKRUSDT', direction: 'SHORT', event_type: 'protocol_critical_exploit', verification_state: 'VERIFIED', confidence_score: 0.77, urgency_score: 0.8, trade_allowed: false, evidence_summary: [{ source_label: 'DefiLlama Hacks', summary: 'DefiLlama hack record: project=DAO Maker Vesting; classification=Protocol Logic; technique=Access Control Exploit', publisher_ts: '2021-09-04T00:00:00Z' }] } }));
  assert.equal(staleDefillama.newsTrade.status, 'NO_TRADE');
  const legacyDefillamaNoDate = parseInboxLine(JSON.stringify({ received_at: new Date().toISOString(), payload: { symbol: 'MKRUSDT', direction: 'SHORT', event_type: 'protocol_critical_exploit', verification_state: 'VERIFIED', confidence_score: 0.77, urgency_score: 0.8, trade_allowed: false, evidence_summary: [{ source_label: 'DefiLlama Hacks', summary: 'DefiLlama hack record: project=DAO Maker Vesting; classification=Protocol Logic; technique=Access Control Exploit' }] } }));
  assert.equal(legacyDefillamaNoDate.newsTrade.status, 'NO_TRADE');
