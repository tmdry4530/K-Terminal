...
function topNewsRecommendations(limit = 3) {
  return (state.signals || [])
    .filter((signal) => signal?.symbol && isFreshLiveSignal(signal) && isHighImpactNews(signal))
  ...
  body.innerHTML = `<div class="news-picks">
    ${picks.map((signal, index) => {
      ...
      return `<button class="news-pick${active}" data-signal-id="${escapeHtml(signal.signalId || '')}">
        <div class="news-pick-top"><span class="badge">LIVE #${index + 1}</span><span class="badge ${trade.className}">${escapeHtml(trade.label)}</span><span class="muted">${escapeHtml(signalAgeHours(signal) < 1 ? `${Math.round(signalAgeHours(signal) * 60)}m 전` : `${signalAgeHours(signal).toFixed(1)}h 전`)}</span></div>
        ...
        <div class="news-pick-reason">${escapeHtml(impactGateReason(signal))} · ${escapeHtml((trade.reasons || []).slice(0, 2).join(', '))}</div>
      </button>`;
    }).join('')}
  </div>
